...
using System.Linq;
using FrostweepGames.Plugins.UniWebConferencePro;
...
namespace FrostweepGames.UniWebConferencePro
{
    public class UniWebConference
    {
        ...
        public void ProcessSpatialAudio(User target)
        {
            ...
            if (SelfUser.Transform != null && target.Transform != null)
            {
                float volume = GetSpatialAudioVolume(SelfUser.Transform.position, target.Transform.position, GeneralConfig.Config.spatialAudioRadius);
                SetAudioVolume(target.Id, volume);
            }
        }

        public float GetSpatialAudioVolume(Vector3 sourcePosition, Vector3 targetPosition, float radius)
        {
            float difference = radius - Vector3.Distance(sourcePosition, targetPosition);
